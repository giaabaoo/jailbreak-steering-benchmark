...
**Angular steering**: Inspired by [Vu et al., 2025](https://arxiv.org/abs/2510.26243), we apply rotation directly to the refusal direction as the steering axis. Activation addition changes both direction and magnitude. We test whether changing direction only works better and whether this closes the gap for SAE max-text steering.
...
`refusal_only`: `h += -1.0 * r` at layer 15 via pre-hook. Direction computed on-the-fly from 8 contrastive pairs.

`refusal_dir_actadd`: `h += -1.0 * r` at layer 15 via pre-hook. Uses the Arditi et al. direction.
...
- Intervention space must match extraction space. The Arditi et al. direction is extracted at pre-hook layer 15. Applying at fwd-hook (layer output) collapses ASR from 89.1% to 37.1%.
...
- SAE max-text reaches 87.2% but this is an oracle upper bound. We tested multiple selection criteria (activation score, semantic similarity) but none consistently predicted which SAE feature would contribute meaningfully. SAE activations are purely correlational, so there is no guarantee any single feature aligns with the harmful intent of the prompt.
